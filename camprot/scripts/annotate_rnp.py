...
def main(argv=sys.argv):
    ...
    tr_only_peptides = set([x for x in list(pep2pro.keys()) if len(pep2pro[x][1])==0])
    ...
    peptide_count = max(list(map(len, list(tmppro2pep.values()))))
    ...
    counts = collections.Counter([len(x) for x in list(new_pep2pro.values())])
